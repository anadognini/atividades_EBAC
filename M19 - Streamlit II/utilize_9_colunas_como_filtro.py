# -*- coding: utf-8 -*-
"""utilize_9_colunas_como_filtro.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1hRbim2GoLec1i-YQN2r0MvN3rxRgdoEG
"""

import pandas as pd
import seaborn as sns
import matplotlib.pyplot as plt

bank_raw = pd.read_csv("/content/bank-additional-full.csv", sep=';')
bank = bank_raw.copy()

"""### Idade"""

max_age = int(bank.age.max())
min_age = int(bank.age.min())

print(f"Max age: {max_age}")
print(f"Min age: {min_age}")

idades = [min_age, max_age]
bank = bank[(bank['age'] >= idades[0]) & (bank['age'] <= idades[1])]

"""### Profissões"""

bank.job.unique().tolist()

jobs_list = bank.job.unique().tolist()
jobs_list.append('all')
print(f"Profissões disponíveis: {jobs_list}")

# jobs_selected = ['housemaid', 'services']
# bank = bank[bank['job'].isin(jobs_selected)].reset_index(drop=True)

jobs_selected = ['all']
jobs_selected

if 'all' in jobs_selected:
  pass
else:
  bank = bank[bank['job'].isin(jobs_selected)].reset_index(drop=True)

"""### 7 filtros"""

bank[['age', 'job', 'marital', 'default', 'housing', 'loan', 'contact', 'month', 'day_of_week', 'y']]

bank.marital.unique().tolist()

bank.default.unique().tolist()

bank.housing.unique().tolist()

bank.loan.unique().tolist()

bank.contact.unique().tolist()

bank.month.unique().tolist()

bank.day_of_week.unique().tolist()

bank.marital.unique().tolist()