...
tips[['net_bill', 'tip_pct', 'pred_tip_pct']].head()

# O atributo .fittedvalues retorna o valor estimado pelo modelo para cada linha,
# com base na regressão tip_pct ~ net_bill.

"""####  2. Obtenha o valor predito para ```tip``` como ```tip_pct * net_bill```"""

tips['pred_tip'] = tips['pred_tip_pct'] * tips['net_bill']
tips[['net_bill', 'tip', 'pred_tip']].head()

"""####  3. Calcule o $R^2$ do modelo para ```tip```"""

# Gorjeta (via percentual predito)

reg_pred = smf.ols('tip ~ pred_tip', data=tips).fit()
reg_pred.summary()

"""####  4. Compare esse $R^2$ com o do item 1

O R² do modelo do item 1 (modelo 1) dado por `tip ~ net_bill` foi de 0.333, enquanto o desse modelo dado por `tip ~ pred_tip` foi de 0.121. Ele tende a ser baixo, pois o modelo de tip_pct explica menos variabilidad. Isso indica uma relação indireta, mais fraca entre as variáveis `tip` e `pred_tip`.
"""